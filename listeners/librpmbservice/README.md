...
# - /usr/local/bin/rpmb_test_client
...
### Test Client
The included test client demonstrates all RPMB operations:

```bash
# Run test client with default device
./rpmb_test_client

# Run with specific RPMB device
./rpmb_test_client /dev/mmcblk0rpmb

# Show help
./rpmb_test_client --help
```

### Test Operations
1. **Key Programming**: Program authentication key
2. **Write Counter**: Get monotonic write counter
3. **Data Write**: Write authenticated data
4. **Data Read**: Read authenticated data
5. **Device Info**: Get RPMB device capabilities
6. **Key Verification**: Verify programmed key
